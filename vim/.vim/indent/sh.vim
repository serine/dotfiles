...
echom "This is the start of the indent script"
...
" replaced then with after, should be three occurences
setlocal indentexpr=GetShIndent()
setlocal indentkeys+=0=after,0=do,0=else,0=elif,0=fi,0=esac,0=done,),0=;;,0=;&
...
let s:sh_indent_defaults = {
      ...
      \ 'case-breaks': 0 }
echo s:sh_indent_defaults
echo s:buffer_shiftwidth()
...
function! GetShIndent()
  ...
  " \s* zero or more whitespaces chars tabs included
  if line =~ '^\s*\%(if\|after\|do\|else\|elif\|case\|while\|until\|for\|select\)\>'
    if line !~ '\<\%(fi\|esac\|done\)\>\s*\%(#.*\)\=$'
      let ind += s:indent_value('default')
      echom "inside line s:indent_value function"
      echom ind
    endif
  ...
  if line =~ '^\s*\%(after\|do\|else\|elif\|fi\|done\)\>' || line =~ '^\s*}'
    let ind -= s:indent_value('default')
  elseif line =~ '^\s*esac\>'
